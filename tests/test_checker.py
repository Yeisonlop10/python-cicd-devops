...
def test_check_urls_request_exception(
    ...
    mocker_requests_get.side_effect = error_exception(f"Simulated {expected_status}")
...
def test_check_urls_multiple_urls(mocker: MockerFixture):
    ...
    mocker_requests_get.side_effect = [
        mocker_response,
        timeout_exception,
        mocker_response_fail,
    ]

    urls = [
        "http://www.success.com",
        "http://www.timeout.com",
        "http://www.servererror.com",
    ]
    results = check_urls(urls)
